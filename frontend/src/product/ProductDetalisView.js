...
function ProductDetailsView({history, match}) {
    ...
    const coords = useSelector(state => state.gpsReducer.coords);
    ...
    function calculateDistance(product) {
        if(coords) {
            ...
            return Math.round(getDistanceFromLatLonInKm(coords.latitude, coords.longitude, product.latitude, product.longitude));
        } else {
            console.log("No coords -> skip")
            return "?";
        }
    }
    return (
